...
class TestData :
    ...
    def disjoint_distribution_patches() :
        ...
        distribution = numpy.zeros( ( 1, 8, 2, 2, 2, 64 ) )
        for p in range( 0, 8 ) :
            for z in range( 0, 2 ) :
                for y in range( 0, 2 ) :
                    for x in range( 0, 2 ) :
    ...
    def overlapping_distribution_patches() :
        ...
        distribution = numpy.zeros( ( 1, 27, 2, 2, 2, 64 ) )
        for k in range( 0, 3 ) :
            for j in range( 0, 3 ) :
                for i in range( 0, 3 ) :
                    for z in range( 0, 2 ) :
...
class DenseLabelTests( unittest.TestCase ) :
    ...
    def test_dense_volume_indices_to_dense_volume_masks( self ) :
        ...
        indices = TestData.index_volume().reshape( ( 1, 4, 4, 4 ) )
        group_by_mask = ( 0, 4, 1, 2, 3 )
...
class SparseLabeltests( unittest.TestCase ) :
    ...
    def test_dense_patch_distribution_to_sparse_patch_distribution( self ) :
        ...
        expected_samples = dense_distribution[ :, :, 1, 1, 1, : ].reshape( 1, 1, 8, 64 )
    ...
    def test_sparse_patch_distribution_to_dense_volume_distribution( self ) :
        ...
        sparse_patches = dense_patches[ :, :, 1, 1, 1, : ].reshape( 1, 27, 64 )
        computed_volume = labels.sparse_patch_distribution_to_dense_volume_distribution( sparse_patches, ( 3, 3, 3 ) )
        expected_volume = TestData.distribution_volume()[ :, 1:, 1:, 1:, : ].reshape( ( 1, 3, 3, 3, 64 ) )
